...
public class SocratesBuilder extends MapBuilder {
    ...
    protected void buildPart(GameData gameData, GameMap gm) {
        ...
        Room sickbay = new SickbayRoom(24, 6,  1, 3, 1, new int[]{1, 16} ,         new double[]{7.5, 1.0, 7.5, 2.0}, RoomType.science );
        gm.addRoom(sickbay, ss13, "port");

        Room gate = new Room(19, "Shuttle Gate", "Gate"   ,10,  1, 2, 3, new int[]{1, 16, 23}, new double[]{10.5, 1.0, 10.0, 2.5}, RoomType.hall );
        gm.addRoom(gate, ss13, "port");
        gm.addRoom(gate, ss13, "front");

        Room frontHall = new Room(23, "Port Hall Aft" ,"FRONT"  ,10,4,3, 2, new int[]{19, 21, 20, 13} , new double[]{11.5, 4.0}, RoomType.hall );
        gm.addRoom(frontHall, ss13, "port");
        gm.addRoom(frontHall, ss13, "front");

        Room portHallFront = new Room(16, "Port Hall", "PORT",4,  2, 6, 1, new int[]{19, 24, 25, 27},  new double[]{4.0, 2.5}, RoomType.hall );
        gm.addRoom(portHallFront, ss13, "port");

        gm.addRoom(new AirLockRoom(25, 3    , 3,  2, 1, 1, new int[]{16, 27}, new double[]{} ), ss13, "port");

        Room panorama = new PanoramaRoom(27, 3,  3, 2, 1, new int[]{25, 16, 8},  new double[]{3.5, 3.0, 4.5, 3.0}, RoomType.hall );
        ...
        Room dorms = new DormsRoom(12, 0, 5, 3, 2, new int[]{8, 5},  new double[]{2.5, 5.0} , RoomType.support);
        gm.addRoom(dorms, ss13, "aft");

        Room chapel = new ChapelRoom(2, 3, 5, 2, 1, new int[]{8, 5}, new double[]{3.5, 5.0}, RoomType.support);
        gm.addRoom(chapel, ss13, "aft");

        Room office = new OfficeRoom(gameData, 14, 1, 7, 2, 1, new int[]{5}, new double[]{}, RoomType.command );
        gm.addRoom(office, ss13, "aft");

        gm.addRoom(new Room( 5, "Aft Hall" , "AFT", 3,  6, 1, 3, new int[]{12, 2, 14, 4, 26}, new double[]{3.5, 6.0, 3.0, 6.5, 3.0, 7.5}, RoomType.hall), ss13, "aft");
        gm.addRoom(new Room( 4, "Airtunnel"           , "", 3,  9, 2, 1, new int[]{5, 6},  new double[]{3.5, 9.0}, RoomType.hall), ss13, "starboard");

        Room green = new GreenhouseRoom(gameData, 3,  6,  10, 5, 2, new int[]{6, 7, 9}  ,  new double[]{6.0, 10.5}  );
        gm.addRoom(green, ss13, "starboard");

        Room aftWalk = new Room(6, "Starboard Walkway", "", 4, 10, 2, 1, new int[]{3, 4}, new double[]{4.5, 10.0}, RoomType.hall);
        ...
        Room sHallAft = new Room( 9, "Starboard Hall"  , "S T A R B O A R D" , 11,  9, 2, 2, new int[]{7, 3, 10, 18}, new double[]{11.0, 10.5, 11.5, 9.0, 12.5, 9.0}, RoomType.hall );
        ...
        Room bar = new BarRoom(gameData, 10, 10, 7, 2, 2, new int[]{9, 18, 13}, new double[]{10.5, 7.0}, RoomType.support);
        gm.addRoom(bar, ss13, "center");

        Room mainHall = new Room(13, "Main Hall", "MAIN",7,  6, 4, 1, new int[]{10, 23, 15, 17, 26},     new double[]{7.5, 6.0, 7.0, 6.5, 10.5, 6.0}, RoomType.hall );
        ...
        Room aiCore = new AIRoom(gameData, 15, 8,  7, 2, 1, new int[]{13} , new double[]{9.0, 7.0}, RoomType.tech );
        gm.addRoom(aiCore, ss13, "center");

        Room bridge = new BridgeRoom(17, 6,  4, 3, 2, new int[]{13}, new double[]{} , RoomType.command );
        gm.addRoom(bridge, ss13, "center");


        Room CQ = new CaptainsQuartersRoom(gameData, 20, 13,  5, 2, 1, new int[]{23}, new double[]{13.0, 5.5}, RoomType.command);
        gm.addRoom(CQ, ss13, "front");

        Room ss = new SecurityRoom(gameData, 18, 12,  7, 2, 2, new int[]{10, 9}, new double[]{12.0, 8.0} , RoomType.security, frontHall);
